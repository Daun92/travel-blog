...
### 2026-02-21: 신도림 폐기 → 부평 산업유산 포스트 전환 — 소재 검증 프로세스 확립

#### 세션 요약

신도림 디깅 포스트의 근본적 약점을 진단하고 폐기, 부평 산업유산으로 주제를 전환하여 발행까지 완료한 세션.

#### 1단계: 신도림 포스트 문제 진단

기존 신도림 포스트(오덕우/niche)의 핵심 문제:

| 문제 | 내용 |
|------|------|
| **가짜 희소성** | "12번 방문 끝에 찾아낸"이라 했지만 테크노마트+동네공원+식당가 — 1번만 가도 다 보이는 것들 |
| **현장에 증거 없음** | 구로공업단지 연결을 주장하지만 건물 자체에 공장 흔적 없음. KTO 이미지도 별도 역사관 사진을 억지로 끌어옴 |
| **검색 의도 불일치** | "신도림 디깅"은 실제 검색량 거의 0. KTO API 검색도 체인점(올리브영, 이마트)만 반환 |
| **문학적 포장 ≠ 콘텐츠** | "산업적 고요함", "빛이 머무는 궤적" 등 수사가 실체의 빈곤을 덮으려 함 |

**교훈**: 디깅 페르소나가 작동하려면 **진짜 숨겨진 레이어가 있는 장소**가 필요. 모든 장소가 포스트가 될 수 있는 것은 아님.

#### 2단계: 대안 주제 탐색 — "한국 산업화의 축약판" 기준

후보 3곳을 **4가지 기준**으로 평가:

```
1. 산업 시대의 물리적 흔적이 지금도 남아있는가
2. 변환이 걸어다니면서 체감 가능한가
3. 네이버 블로그에 이미 넘치는 주제가 아닌가
4. 디깅/해설이 파고들 미시적 발견이 있는가
```

| 후보 | 흔적 가시성 | 디깅 신선도 | 콘텐츠 갭 | 판정 |
|------|-----------|-----------|----------|------|
| 문래동 철공소 | 현재진행형 (불꽃, 소리) | 중간 (카페 쪽 포화) | 중간 | 차선 |
| 영등포 경성방직 | 부분적 (타임스퀘어가 덮음) | 낮음 | 작음 | 탈락 |
| **인천 부평** | **유적 수준 보존** | **높음** | **완전 공백** | **채택** |

**부평 채택 이유**: 4개 시대(일제 조병창→미군기지→자동차 공장→지하상가)의 물리적 증거가 전부 남아있고, 통합 서사 관점의 블로그 포스트가 **제로**.

#### 3단계: 부평 리서치 (3개 병렬 에이전트)

| 에이전트 | 담당 | 핵심 발견 |
|---------|------|----------|
| 조병창/캠프마켓 | 일제·미군 레이어 | 지하호 24개 중 1개만 공개, 월 1회 무료 투어. 미쓰비시 줄사택 2024.8 국가등록문화유산 858호 |
| 지하상가/시장/GM | 산업·상업 레이어 | 부평모두몰 1,408점포 WRA 세계기록. GM 이름 7번 변경. 깡시장=미군 잉여물자 기원 |
| 관광 인프라/갭 분석 | 메타 분석 | "걷다보니 부평" 공식 4코스 존재. 통합 산업유산 서사 블로그 = 완전 공백 |

#### 4단계: 서사 설계 — "도시의 지층을 걷다"

페르소나를 오덕우(niche) → **한교양(informative)**으로 전환. 이유: 역사·산업 변천사 해설은 교양 페르소나의 본령.

**서사 구조**: 지표면에서 시작해 읽어 내려갈수록 시간이 깊어지는 **지층 메타포**.

```
지표면 (2026)     부평모두몰 1,408개 점포, 12만 명
  ↓ 1층 지층      깡시장 — 미군 잉여물자의 골목 경제학
  ↓ 2층 지층      GM 굴뚝 — 새나라자동차(1962)부터 7번 바뀐 간판
  ↓ 3층 지층      캠프마켓 — 84년간 닫힌 문 (역설적 보존)
  ↓ 암반층        지하호 150m — 정 자국과 손톱만 한 종유석
```

마감에서 다시 지표면으로 돌아와 "같은 풍경이 다르게 보인다"로 원환 구조 완성.

#### 5단계: 이미지 전략

| 소스 | 장수 | 대상 |
|------|------|------|
| KTO 실사 | 3 | 부평시장 (detailImage2로 9장 중 3장 선별) |
| AI 스틸컷 | 5 | GM공장, 캠프마켓, 줄사택, 지하호 내부, 지하호 출구 |
| AI 일러스트 | 2 | 지층 단면도(diagram), 마감 무드보드(moodboard) |
| AI 커버 | 1 | 한교양 블루 관인 포함 |

**Batch API 이슈**: 8장 배치 요청 → PENDING 상태에서 10분 타임아웃. 순차 모드 폴백으로 전환하여 전량 성공.
**파일명 이슈**: `saveImage()` optimize 옵션이 `.jpg` 확장자를 자동 추가하여 `.jpeg.jpg` 이중 확장자 발생. 리네임으로 해결.

#### 6단계: 팩트체크 (82%)

자동 추출기는 3개만 잡았으나 수동으로 15개 핵심 클레임 식별 후 WebSearch 검증.

| 클레임 | 결과 | 수정 |
|--------|------|------|
| 강제동원 "약 1만 5천 명" | nuance | → "9,000명 실측 + 1.5만 추가 동원 계획" |
| 캠프마켓 "존치가 결정" | nuance | → "시민참여위가 존치를 권고" |
| 포니 비교 "14년 앞섬" | nuance | → "13년 앞섬" (1975.12 양산 기준) |
| 나머지 12개 | verified | — |

#### 7단계: AEO + GEO → 발행

- AEO: FAQ 5개 + Schema.org 3개(FAQPage, Article, BreadcrumbList)
- GEO: 한교양 도입 요약 blockquote (장소·교통·투어 정보)
- `draft: false` → Hugo 빌드 → blog repo push

#### 인사이트 — 소재 검증 체크리스트 (신규 프로세스)

이번 세션에서 확립된, 포스트 소재 선정 전 검증 기준:

```
□ 현장에 물리적 증거가 있는가? (사진으로 보여줄 수 있는가)
□ KTO API에 등록된 관광자원이 있는가? (실사 이미지 확보 가능성)
□ 검색 의도와 매칭되는가? (누가, 왜 이 키워드를 검색하는가)
□ 기존 네이버 블로그와 차별되는 관점이 있는가? (콘텐츠 갭)
□ 선택한 페르소나가 이 소재에 적합한가? (디깅≠모든 곳에 적용 가능)
```

**신도림이 이 5개 중 0개를 통과하지 못했고, 부평은 5개 전부 통과했다.**

#### 최종 산출물

| 항목 | 내용 |
|------|------|
| 발행 포스트 | `bupyeong-industrial-heritage-walking` (한교양) |
| 폐기 포스트 | `sindorim-factory-zone-deep-digging` (삭제 완료) |
| 팩트체크 | 82% (15개 중 verified 12, nuance 수정 3, false 0) |
| 이미지 | KTO 3 + AI 8 = 11장 |
| 커밋 | blog `9bc82df`→`c1556d9`, main `cce5999` |

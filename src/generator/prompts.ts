...
/**
 * 페르소나별 GEO 도입부 요약 톤 가이던스
 */
function buildGeoIntroGuidance(persona?: Persona): string {
  if (!persona) {
    return `> **{주제} 핵심 요약** — {장소}는 {위치}에 있으며, {핵심 특징 1-2개}. {비용/시간 등 실용 정보}.`;
  }

  const toneGuides: Record<string, string> = {
    viral: `"결론 먼저" 스타일 — 핵심만 뽑은 1줄 요약 + 실용 수치(순위, 가격, 시간).
      예: > **서울 봄꽃 명소 TOP 5 핵심** — 석촌호수(벚꽃 4월 초중순), 남산(개나리 3월 말), 여의도 윤중로(벚꽃 축제 4/5~4/11). 전부 지하철 도보 10분 이내, 입장료 무료.`,

    friendly: `"실용 정보" 스타일 — 어디에, 얼마에, 누구에게 좋은지 한눈에.
      예: > **양평 당일치기 핵심** — 서울에서 ITX 50분, 두물머리(무료)+세미원(5,000원)+용문사(2,500원) 코스. 가족·연인 모두 OK.`,

    informative: `"학술적 개요" 스타일 — 역사적·문화적 맥락과 핵심 의의를 담은 요약.
      예: > **불국사 핵심 요약** — 경주 불국사는 751년(경덕왕 10년) 창건된 유네스코 세계문화유산. 다보탑·석가탑·청운교 등 국보 7점 보유. 입장료 성인 6,000원, 09:00-17:00.`,

    niche: `"발견 요약" 스타일 — 남들이 모르는 핵심 포인트를 짧게.
      예: > **을지로 인쇄골목 핵심** — 을지로3가역 4번 출구, 50년 된 활판인쇄소 3곳이 현역. 명함 100장 1만원부터, 사전 예약 필수.`
  };

  const personaId = persona.id || 'friendly';
  return toneGuides[personaId] || toneGuides.friendly;
}

/**
 * 주제 범위 가드레일 — 프레이밍 유형별 섹션 제약
 */
function buildTopicScopeGuard(topic: string, type: 'travel' | 'culture', framingType?: string): string {
  const base = `   - ⛔ 모든 H2 섹션은 "${topic}"에 직접 관련되어야 합니다.
   - ⛔ 주제 무관 범용 섹션("주변 맛집", "근처 볼거리", "연계 코스", "함께 가볼 곳") 추가 금지. 주제 자체의 깊이를 높이세요.
   - 각 섹션 안에서 다른 장소/주제로 갑작스러운 전환 금지.`;

  const framingConstraints: Record<string, string> = {
    list_ranking: `   - 리스트 항목은 모두 "${topic}" 범주 안에서 선정하세요. 주제 밖의 장소/항목을 억지로 채우지 마세요.`,
    deep_dive: `   - 단일 주제를 심층 탐구하세요. 인접 주제로 확산하거나 범용 정보로 분량을 채우지 마세요.`,
    experience: `   - 서사 흐름의 일관성을 유지하세요. 시간순 또는 동선순으로 자연스럽게 전개하되, 주제를 벗어나는 장소를 삽입하지 마세요.`,
    comparison: `   - 비교 대상은 "${topic}"이 직접 다루는 항목으로만 한정하세요.`,
    local_story: `   - 로컬 스토리는 "${topic}" 현장의 이야기에 집중하세요. 해당 동네를 벗어나는 장소를 추가하지 마세요.`,
    niche_digging: `   - 디깅 대상은 "${topic}" 하나에 집중하세요. 유사 장소나 비교 대상을 나열하지 마세요.`,
  };

  const constraint = framingType ? framingConstraints[framingType] || '' : '';
...
${framingGuide}
` : ''}${collectedData ? `
## 수집 데이터 (주제 범위 내에서만 활용)
⚠️ 아래 데이터 중 "${topic}"과 직접 관련된 항목만 사용하세요.
같은 지역이라도 주제와 무관한 시설/행사는 언급하지 마세요.
${collectedData}
` : ''}
## 작성 지침
...
2. **톤**: ${toneGuide}
3. **구조** (필수 준수):
${buildTopicScopeGuard(topic, 'travel', framingType)}
   - 매력적인 도입부 (왜 이 장소인가?)
   - 상세한 여행 정보 (위치, 교통, 비용)
...
7. **GEO 최적화** (AI 검색 엔진 인용 친화):
   a) **도입부 팩트 요약**: 제목 직후, 첫 H2 이전에 블록인용(>) 형태로 2-3문장 핵심 정보 요약 작성.
      장소명/주제를 명시적으로 포함하고, 한 문장만 읽어도 "누가/무엇/어디/얼마"를 알 수 있도록.
      ${buildGeoIntroGuidance(persona)}
   b) **섹션 첫 문장 자족성**: 각 H2 섹션의 첫 문장은 해당 섹션의 장소/주제를 대명사 없이 명시적으로 언급.
      - 나쁜 예: "여기는 안국역에서 5분 거리입니다."
      - 좋은 예: "북촌한옥마을은 지하철 3호선 안국역 2번 출구에서 도보 5분 거리에 있습니다."
   c) **완결형 정보 문장**: 주요 팩트(가격, 시간, 위치)를 독립 문장으로 작성하여,
      문맥 없이 해당 문장만 발췌해도 의미가 통하도록.

⚠️ **이미지는 별도 시스템이 자동 배치합니다. 본문에 [IMAGE:...] 마커를 삽입하지 마세요.**
...
**slug 규칙**:
- 주제의 핵심을 영문으로 번역/요약한 URL용 슬러그 (예: "양평 vs 가평 당일치기" → "yangpyeong-vs-gapyeong-day-trip")
- 소문자, 하이픈(-) 구분, 3~6단어 (예: "gyeongju-bulguksa-temple-guide")
- 한국 지명은 로마자 표기 (예: 경주→gyeongju, 서울→seoul, 제주→jeju)
- 숫자/영문 키워드(TOP, BEST, vs)는 그대로 사용
...
2. **톤**: ${toneGuide}
3. **구조** (필수 준수):
${buildTopicScopeGuard(topic, 'culture', framingType)}
...
7. **GEO 최적화** (AI 검색 엔진 인용 친화):
   a) **도입부 팩트 요약**: 제목 직후, 첫 H2 이전에 블록인용(>) 형태로 2-3문장 핵심 정보 요약 작성.
      ...
      ${buildGeoIntroGuidance(persona)}
   b) **섹션 첫 문장 자족성**: 각 H2 섹션의 첫 문장은 해당 섹션의 장소/주제를 대명사 없이 명시적으로 언급.
      - 나쁜 예: "이곳에서 다양한 전시를 감상할 수 있습니다."
      - 좋은 예: "서울시립미술관 서소문본관은 매주 목요일 야간 개방(21:00까지)으로 퇴근 후 관람이 가능합니다."
   c) **완결형 정보 문장**: 주요 팩트(가격, 시간, 위치)를 독립 문장으로 작성하여,
      문맥 없이 해당 문장만 발췌해도 의미가 통하도록.
...
**slug 규칙**:
- 주제의 핵심을 영문으로 번역/요약한 URL용 슬러그 (예: "서울 3월 전시 공연 TOP 5" → "seoul-march-exhibitions-top-5")
- 소문자, 하이픈(-) 구분, 3~6단어 (예: "jeju-yeongdeung-haenyeo-festival")
- 한국 지명은 로마자 표기 (예: 경주→gyeongju, 서울→seoul, 제주→jeju)
- 숫자/영문 키워드(TOP, BEST, vs)는 그대로 사용
